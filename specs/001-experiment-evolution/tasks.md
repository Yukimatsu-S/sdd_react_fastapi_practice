...
- [ ] T002 Create the React/Vite/TypeScript application scripts and dependencies in `frontend/package.json`, `frontend/package-lock.json`, `frontend/vite.config.ts`, and `frontend/tsconfig.json`; 対応: 共通 (US1-US4); 依存: なし; 検証: `cd frontend && npm ci && npm run build` completes with the empty application entry point.
...
- [ ] T005 Configure backend test discovery and test-only environment selection in `backend/pytest.ini`; 対応: 共通 (US1-US4); 依存: T001, T003, T004; 検証: `cd backend && uv run pytest --collect-only` discovers `tests/unit`, `tests/integration`, and `tests/contract` without using a production database URL.
- [ ] T006 Configure Vitest, React Testing Library setup, and the browser-like test environment in `frontend/vitest.config.ts` and `frontend/src/test/setup.ts`; 対応: 共通 (US1-US4); 依存: T002; 検証: `cd frontend && npm test -- --run` starts Vitest and completes with zero test files before feature tests are added.
...
- [ ] T010 [P] Write a failing API error-envelope and trace-ID contract test in `backend/tests/contract/test_error_responses.py`; 対応: 共通 (US1-US4); 依存: T008; 検証: before T017-T018, `cd backend && uv run pytest tests/contract/test_error_responses.py` fails because versioned error handling is absent.
...
- [ ] T012 Implement validated environment loading and application settings in `backend/app/config.py`; 対応: 共通 (US1-US4); 依存: T003, T009-T011; 検証: the configuration rejects missing required connection settings and `cd backend && uv run pytest tests/integration/test_initial_schema.py` progresses past settings initialization.
...
- [ ] T017 [P] Implement structured API exceptions, `code`/`message`/`traceId` responses, and trace-ID logging in `backend/app/api/errors.py`; 対応: 共通 (US1-US4); 依存: T010, T012; 検証: `cd backend && uv run pytest tests/contract/test_error_responses.py` passes for validation, not-found, conflict, and upstream-failure responses.
- [ ] T018 Create the `/api/v1` router registration, request-session lifecycle, and exception middleware in `backend/app/api/router.py` and `backend/main.py`; 対応: 共通 (US1-US4); 依存: T013, T016, T017; 検証: `GET /api/v1/unknown` returns the T017 error envelope and the root route is no longer the only API surface.
...
- [ ] T039 [P] [US1] Implement MLflow Run search, synchronization, and best-step placeholder routes in `backend/app/api/routes/runs.py`; 対応: US1; 依存: T035, T037; 検証: the Run-search and sync portions of `cd backend && uv run pytest tests/contract/test_runs_and_evolution_steps.py` pass.
...
- [ ] T048 [US1] Add list, create, and detail navigation routes and application bootstrap in `frontend/src/routes/AppRoutes.tsx` and `frontend/src/main.tsx`; 対応: US1; 依存: T045, T047; 検証: `cd frontend && npm run build` completes and direct navigation to create and detail routes resolves.
