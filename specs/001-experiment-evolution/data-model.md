...
`id` (PK), `run_id` (FK to `run_snapshot`), `name`, `value` (double), `step` (bigint), `timestamp` (datetime(6)). Unique `(run_id, name, step, timestamp, value)` prevents accidental duplicate capture while retaining complete metric histories. `INDEX(run_id, name, value)` supports `max(accuracy)`.

### `dataset_input`

`id` (PK), `run_id` (FK to `run_snapshot`), `ordinal` (int), `name`, `digest`, `source_type`, `source_uri`, `schema`, `profile`, `context`, and `raw_metadata` (json nullable). Unique `(run_id, ordinal)`. The service exposes the identifier fields supplied by MLflow and marks an absent field unavailable.
...
2. If the Snapshot is pending and the Run is terminal, retrieve all Parameters, metric histories, and dataset inputs before opening the transaction. If it is still active, retrieve only current reference metadata.
...
- **Evolution Step list**: Evolution Step ID, purpose, hypothesis, parent/result Run summary with last-known Run name, current status, synchronization time, and Snapshot state, timestamps, and a compact comparison summary. It reads only MySQL and does not trigger synchronization. The summary reports comparison availability, Parameter change count, best parent/result `accuracy` and delta when available, and dataset status as changed, unchanged, or unavailable. It does not include the full Parameter or dataset differences.
- **Detail**: return current Evolution Step fields, each linked Run Reference and optional Snapshot, all captured Parameters, metric observations, dataset inputs, and ordered history from MySQL. It does not call MLflow. After this response is rendered, the frontend invokes the separate synchronization action and reloads detail on success.
