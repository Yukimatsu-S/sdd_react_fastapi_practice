...
1. **Create and attach**: Create an Evolution Step with non-empty purpose and hypothesis. Select an MLflow parent Run and a distinct result Run. Expect imported Parameters, all metric observations, dataset data, and current links in the detail response. Confirm the history records each later edit.
...
6. **MLflow failure boundary and snapshot reuse**: Make the MLflow gateway time out or return an unreadable selected Run that has no saved snapshot. Expect the attach/update API to return `502` with a trace ID, leave existing Evolution Step links untouched, and allow saved details/Lineage to continue reading from MySQL. Then associate a Run that already has a saved snapshot and confirm the operation reuses its local snapshot payload; only Run-name synchronization may call MLflow, and its failure must not overwrite the snapshot or block the association.
7. **Run-name synchronization**: Rename a linked Run in MLflow, then open its Evolution Step detail. Expect the current name and `runNameSyncedAt` to be returned while Parameters, Metrics, Dataset inputs, status, execution timestamps, and comparison results remain unchanged. Next make MLflow unavailable and open the detail again. Expect HTTP `200`, the last-known name and synchronization time, unchanged snapshot data, and a Run-name synchronization warning.
