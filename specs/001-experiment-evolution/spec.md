...
- Q: 紐付け後にMLflow上のRun名が変更された場合、保存済みRun情報をどのように扱いますか？ → A: Run名だけを現在の表示情報として、Run候補の取得時、Runの紐付け・変更時、およびEvolution Step詳細を開いた時にMLflowから同期する。Parameters、Metrics、データセット識別情報などのSnapshotは初回取得時点のまま更新しない。詳細表示時にRun名を同期できなくても保存済み情報を表示し、最後に同期できたRun名と同期日時を確認できるようにする。
...
5. **Given** Evolution Stepに紐付くRunの名前がMLflow上で変更されている状態、**When** 開発者がそのEvolution Stepの詳細を開く、**Then** 現在のRun名と同期日時が表示され、保存済みのParameters、Metrics、およびデータセット識別情報は変更されない。
6. **Given** Evolution Stepに保存済みRun情報がありMLflowへ接続できない状態、**When** 開発者がそのEvolution Stepの詳細を開く、**Then** 詳細表示は失敗せず、最後に同期できたRun名と同期日時を含む保存済み情報、およびRun名を同期できなかった状態を確認できる。
...
- 保存済みRunを含むEvolution Stepの詳細表示時にMLflowへ接続できない場合、Run名の同期だけを失敗として扱い、保存済みの詳細、比較、およびLineageの閲覧を妨げない。
...
- **FR-011**: システムは、Evolution Stepに手動で紐付けたRunについて、取得したRun情報、実測条件、評価指標、およびデータセット識別情報を記録し、後から確認できるようにしなければならない。
...
- **FR-016**: システムは、派生元Runが指定されたEvolution Stepについて、派生元Runと実行結果Runのデータセット名、バージョンなどの取得済み識別情報の一致または相違を比較表示できるようにしなければならない。
...
- **FR-025**: システムは、Run候補の取得時、Runの新規紐付け・変更時、および紐付いたEvolution Stepの詳細表示時に、MLflow上の現在のRun名を取得し、保存済みRunについてはRun名と最終同期日時を更新しなければならない。Run名は表示用情報であり、Run IDは変更してはならない。
- **FR-026**: システムは、一度保存したRunのParameters、Metrics、データセット識別情報、およびその他のSnapshot情報を自動更新してはならない。Run名の同期によって比較結果またはLineageが変化してはならない。
- **FR-027**: システムは、Evolution Step詳細表示時のRun名同期に失敗した場合でも、最後に同期できたRun名と同期日時を含む保存済み情報を表示し、Run名を同期できなかった状態を利用者に示さなければならない。保存されていないRunを初めて紐付ける際に必要な情報を取得できない場合は、紐付けを完了してはならない。
...
- **Run**: 外部の学習コードがMLflowに記録した一回の学習の記録。Run IDを不変の識別子とし、Run名はMLflow上の変更を同期する表示用情報として扱う。実行状態、実測条件、評価指標、データセット識別情報を持ち、0件以上の後続Evolution Stepの派生元または0件もしくは一つのEvolution Stepの実行結果となり得る。
- **Run Snapshot**: Runを最初に紐付けた時点で保存するParameters、Metrics、データセット識別情報、およびその他の実行情報。比較と振り返りの根拠として再利用し、表示用のRun名とその同期日時を除いて自動更新しない。
...
- Run候補の取得時はMLflow上の現在のRun名を表示する。すでに保存済みのRunと同じRun IDを取得した場合は、保存しているRun名と最終同期日時を更新する。
- Runの新規紐付け・変更時とEvolution Step詳細表示時にも、対象Runの現在のRun名を同期する。詳細表示時の同期は補助的な処理とし、失敗しても保存済み情報を表示する。一覧、比較、およびLineageの表示ではMLflowへ問い合わせず、最後に同期できたRun名を利用する。
- 一度保存したRun SnapshotのParameters、Metrics、データセット識別情報、実行状態、実行日時、および未加工メタデータは更新しない。Run名とその最終同期日時だけを変更可能な表示用情報として扱う。
- データセット識別情報は、MLflowから取得できるデータセット名、バージョンなどを表示および比較の対象とする。定性的なデータ変更の説明は、Evolution Stepの変更内容として記録する。
- 一つのEvolution Stepが持つ派生元Runは0件または1件、実行結果Runは0件または1件とする。一つのRunを実行結果として紐付けられるEvolution Stepは0件または1件とする。実行結果Runは外部での学習完了後に手動で紐付け、未設定のEvolution Stepも登録できる。
