...
- Q: Evolution Stepの変更履歴はいつから記録しますか？ → A: 新規登録自体は履歴に含めず、登録完了後に目的、仮説、変更内容、派生元Run、または実行結果Runの値が実際に変わった編集を記録する。同じ値の再送や、検証・競合などにより失敗した操作は記録しない。
...
- Q: Evolution Step一覧は、どの順序と単位で表示しますか？ → A: 作成日時の降順、作成日時が同じ場合はEvolution Step IDの降順で20件ずつ表示する。続きがある場合だけ次の20件を取得するための値を返し、その値は利用者へ直接入力させない。新しいEvolution Stepが途中で登録されても、取得済みページの続きには混在させず、一覧の再読み込み後に先頭から表示する。
...
- Q: 学習中のRunを紐付けた場合、Snapshotをいつ確定しますか？ → A: Runの参照関係は学習中でも保存し、Snapshotを未確定として表示する。Evolution Step詳細を開いた時に対象Runの状態を自動確認し、RunがFINISHED、FAILED、またはKILLEDの終了状態なら、その時点のParameters、最良stepのMetrics、データセット識別情報などを一度だけ取り込んでSnapshotを確定する。RUNNINGまたはSCHEDULEDなら未確定のままとする。確定後のSnapshotは更新せず、Run名、現在状態、現在の開始・終了日時、および最終同期日時だけをRun Referenceの表示情報として同期する。
...
- Q: 比較全体と個別項目の比較可否、および比較できない理由をどのように表しますか？ → A: 派生元Runと実行結果Runの両方に確定済みSnapshotがある場合、比較全体は利用可能とする。accuracyまたはDataset Inputだけを比較できない場合も、利用可能なParameterなどは表示し、該当項目だけを利用不可として理由を示す。同じレスポンスオブジェクト内では、利用可能な状態と利用不可理由を同時に返さず、利用不可な状態では定義済みの理由を必ず返す。
- Q: MLflow Metricのstepはどの範囲を受け入れますか？ → A: MLflowが許可する符号付き64bit整数をそのまま受け入れ、負数、順不同、飛び番も保持する。Mondel独自の0以上という制約は設けない。
...
6. **Given** Evolution Stepの登録後に、Runの紐付け有無にかかわらず目的、仮説、変更内容、派生元Run、または実行結果Runの値が変更されている状態、**When** 利用者が変更履歴を確認する、**Then** 成功して値が実際に変わった各編集の対象項目、編集前の内容、編集後の内容、編集日時を時系列順に確認できる。
...
18. **Given** Snapshot未確定のRunがMLflow上でRUNNINGまたはSCHEDULEDの状態、**When** 利用者がEvolution Step詳細を開く、**Then** Run名、現在状態、現在の開始・終了日時、および最終同期日時が同期され、Snapshotは未確定のまま表示される。
...
5. **Given** Evolution Stepに紐付くRunの表示情報がMLflow上で変更されている状態、**When** 開発者がそのEvolution Stepの詳細を開く、**Then** 保存済み詳細が表示された後にRun情報が自動同期され、現在のRun名、状態、開始・終了日時、および同期日時を確認できる。確定済みのParameters、最良stepのMetrics、およびデータセット識別情報は変更されない。
...
7. **Given** 21件以上のEvolution Stepが登録されている状態、**When** 開発者が一覧を表示してから続きを取得する、**Then** 作成日時の新しい順で最初の20件と重複しない次の20件を確認でき、最後のページでは続きがないことを確認できる。
...
- **FR-018**: システムは、Evolution Step、現在のRun紐付け、関連するRun情報、実測条件、評価指標、データセット識別情報、変更内容、および変更履歴を利用終了後も保持しなければならない。Lineageそのものを保存するのではなく、保存済みの現在のRun紐付けから再構成し、後から確認できるようにしなければならない。
- **FR-019**: システムは、派生元がない、実行結果Runが未設定である、Snapshotが未確定である、accuracyが記録されていない、比較可能なDataset Inputが不足している、またはMLflowからRun情報を取得できない場合に、その状態と理由を利用者に明確に示さなければならない。派生元Runと実行結果Runの両方に確定済みSnapshotがある場合は比較全体を利用可能とし、accuracyまたはDataset Inputだけを比較できない場合もParameterなどの利用可能な比較結果を表示しなければならない。同じレスポンスオブジェクト内では、利用可能な状態の場合は利用不可理由を持たず、利用不可な状態の場合は定義済みの理由を持たなければならない。
...
- **FR-021**: システムは、Evolution Stepの新規登録自体を変更履歴に含めず、登録完了後に目的、仮説、変更内容、派生元Run、または実行結果Runの値が実際に変わった編集について、対象項目、編集前と編集後の内容、編集日時を変更履歴として保持し、編集日時の昇順で確認できるようにしなければならない。編集日時が同一の場合は内部の変更履歴IDの昇順とし、この内部IDを利用者へ表示してはならない。同じ値の再送および完了しなかった操作の履歴を作成してはならない。
...
- **FR-026**: システムは、Evolution Step詳細表示時に、紐付いたRunの現在の名前、状態、開始・終了日時、および最終同期日時をRun Referenceへ自動的に同期しなければならない。Snapshot未確定のRunがFINISHED、FAILED、またはKILLEDである場合は、その時点のRun情報、Parameters、最良stepのMetrics、およびデータセット識別情報を取得し、Snapshotを一度だけ確定しなければならない。
...
- **FR-030**: システムは、Evolution Step一覧を作成日時の降順、同一作成日時ではEvolution Step IDの降順で20件ずつ返し、続きがある場合だけ次の候補群を取得するための値を提供しなければならない。続きの取得では、直前に返した最後のEvolution Stepより古い項目だけを返さなければならない。
