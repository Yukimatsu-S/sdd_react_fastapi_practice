...
- The migration context fixture is implemented in `backend/tests/integration/test_initial_schema.py`. It loads `migrations/versions/001_initial_schema.py` and invokes `upgrade()` within Alembic's `Operations.context(MigrationContext.configure(connection))`. This tests the revision body independently of T014's CLI environment; it does not test revision discovery, `alembic_version` bookkeeping, CLI upgrade or downgrade. The guarded context is entered inside test bodies so missing revisions are assertion failures rather than collection/setup errors. Run these tests exclusively against the dedicated DB; never run concurrently.
