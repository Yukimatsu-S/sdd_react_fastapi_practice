...
- **Decision**: Import and persist a point-in-time Run snapshot only when a user selects a Run that has no saved snapshot. If the Run already has a saved snapshot, reuse it without calling MLflow again or overwriting it. For a first import, complete MLflow retrieval before committing the new snapshot and Evolution Step association in one database transaction. Details, differences, and Lineage read local data only. Automatic and manual snapshot refresh are out of scope for the initial MVP.
- **Rationale**: Historical review survives MLflow outages and preserves the exact observed values that motivated an Evolution Step. Reusing an immutable snapshot prevents a later association from changing data already used by earlier Evolution Steps. Atomic attach prevents an Evolution Step from pointing at a partially imported Run.
- **Alternatives considered**: Live reads on every detail request were rejected because outages would make saved Evolution Steps unavailable and values could drift. Replacing an existing snapshot on re-import was rejected because it would change historical comparisons for every Evolution Step that references the same Run. Background polling and manual refresh were rejected because the feature does not require refresh behavior and it complicates audit meaning.

## Data persistence

- **Decision**: Use MySQL 8.0+ and SQLAlchemy 2.x with normalized `evolution_step`, `run_snapshot`, `run_parameter`, `metric_observation`, `dataset_input`, and append-only `evolution_step_history` tables. Save MLflow identifiers and values together with import timestamps.
