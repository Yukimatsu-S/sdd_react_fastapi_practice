...
- 開始前に専用DBへの接続を確認し、作成対象の8テーブルと`alembic_version`（適用済みリビジョンを記録するテーブル）の有無を調べる。対象があれば名前とこの復旧手順を表示し、DBを変更せず停止する。
- 対象がなければマイグレーションを適用して検証する。通常終了・テスト失敗では今回の実行が作成した対象だけを後片付けする。適用途中の失敗も後片付けの対象とし、開始前からあるものや無関係なテーブルは削除しない。
- 同じDBを使うアプリや別のテストを同時実行しない。存在確認後に別プロセスが作成すると所有者を判断できなくなる。
- 行の変更はrollbackで取り消すが、MySQLのテーブル作成・削除は同じ方法で戻せない。例外を確認する`pytest.raises()`も後片付けを行わない。
- 強制終了や後片付けの失敗では対象が残る可能性がある。失敗を隠して再実行せず、次の手順で確認する。

### 手順1：実行場所と対象を確認する

全体の流れは「確認 → 保存要否の判断 → バックアップ付き初期化 → 空の状態を確認 → テスト再実行」。元のデータへ戻す場合だけ、後述の復元を使う。

前提はREADMEのセットアップが済んでいること（Docker、uv、バックエンド依存関係）。同じテストDBを使うアプリ・pytest・マイグレーションを停止する。自動ロックで他プロセスを排除する仕組みではない。

リポジトリ直下のターミナルで次を実行する。
...
以降もこの`backend`のターミナルを使う。`uv run python`はプロジェクトのPythonで指定スクリプトを実行する。`inspect`は確認だけの操作で、テーブルを変更しない。

出力の`Target: Compose mysql-test / mondel_test`、各テーブルの行数、存在する場合は`Migration revision`を確認する。空なら`{}`と表示され、初期化せず手順4へ進める。

対象はこのリポジトリのComposeサービス`mysql-test`内の`mondel_test`に固定。コンテナ内のクライアントを使うためMacへのMySQLクライアント導入は不要。接続情報はコンテナのテスト用環境変数を利用し、通常アプリの接続URLは読まない。

### 手順2：保持が必要・不明なら削除しない

既存データを破棄してよいかは人が判断する。残す場合は、次でバックアップだけ作成する。

```bash
uv run python scripts/recover_test_schema.py backup
```

`Backup:`に保存場所、`Restore command`にそのバックアップ専用の復元コマンドが出る。保存先は`backend/.local/schema-recovery/日時-識別子/`。SQL本体と検証用ファイルをセットで保持する。このフォルダはGit対象外だが暗号化されていないため、共有・コミットしない。

保持が必要・不明な場合はここで停止し、出力と保存場所をプロジェクト担当者へ共有する。別DBへの切り替えは既存fixtureの接続先制限も変える必要があるため、この復旧操作では行わない。

### 手順3：不要と確認したテスト用構造を初期化する

```bash
uv run python scripts/recover_test_schema.py reset
```

表示された対象を確認し、削除を承認する場合だけ、入力待ちで次を正確に入力してEnterを押す。

```text
DELETE mondel_test schema
```

これはシェルコマンドではなく確認入力。別の入力なら変更せず停止する。承認後も、スクリプトはまずバックアップを作成し、成功した場合だけ8つの製品テーブルと`alembic_version`のうち存在するものを依存順に削除する。DB本体やDockerボリュームは削除せず、外部キーチェックも無効にしない。

`Reset complete`が成功の目印。テーブルと中のデータは削除されるが、表示されたバックアップから復元できる。バックアップが失敗した場合、削除には進まない。

### 手順4：空の状態を確認し、テストを再実行する

```bash
uv run python scripts/recover_test_schema.py inspect
uv run --env-file .env.example pytest tests/integration/test_initial_schema.py
```

`inspect`で`{}`を確認する。`--env-file .env.example`はテスト用環境設定を明示的に読み込む指定。復旧はテーブル残存を解消するだけで、実装不足を直すものではない。現在はT012/T013等が未完了なので、テスト成功はまだ期待しない。

### 元のデータを戻したい場合

バックアップ作成時に表示された`Restore command`を、そのまま同じターミナルに貼り付けて実行する。形は`uv run python scripts/recover_test_schema.py restore --backup-dir 保存先の絶対パス`。保存先を忘れた場合は`ls -dt .local/schema-recovery/*/`で一覧を表示し、今回記録した日時のフォルダを指定する。

復元先は空でなければ停止する。既存データへ上書きしない。復元するSQLは自分がこのスクリプトで生成した信頼できるものだけを使う。チェックサムは破損検出であって、他人のSQLの安全性を保証するものではない。

確認入力に`RESTORE mondel_test backup`を入力する。`Restore complete`ならテーブル名と行数がバックアップの記録に一致している。全データ内容の完全一致まで検証しているわけではない。復元後は元のテーブルが戻るので、空の状態が必要なマイグレーションテストは再び停止する。

### エラー別の対応

| 表示・状況 | 次の操作 |
|---|---|
| Docker接続失敗・コンテナ停止 | リポジトリ直下で手順1の`docker compose ... up`を実行し、`backend`へ戻って`inspect`を再実行 |
| `confirmation` | キャンセル扱いで変更なし。実行が必要ならコマンドをやり直す |
| バックアップ失敗・空のバックアップ | 削除前に停止。空き容量とコンテナ起動を確認し、`inspect`から再開 |
| `Unexpected object` / `Unsupported objects` | 想定外の表・ビュー・トリガー等を検出。削除しない。エラー内容を担当者へ共有して対象範囲を再設計する |
| `checksum mismatch` | バックアップ破損・変更の可能性。SQLを手修正せず元のバックアップを確認。復元しない |
| `empty database` | 先に`inspect`。残存データも破棄してよい場合だけ`reset`し、元の復元コマンドを再実行 |
| 削除途中の失敗・タイムアウト | DDLは一括rollbackされない。元のバックアップを保持し`inspect`。不要な残骸だけなら`reset`を再実行する。依存関係エラーが再発する場合は停止して担当者へ共有 |
| 復元途中の失敗 | 元のバックアップを保持する。部分復元のデータだけと確認できたら`reset`で片付け、**元の**バックアップの復元コマンドを再実行。失敗が続く場合は担当者へ共有 |
...
```bash
uv run pytest tests/unit/test_schema_recovery.py -q
uv run pytest tests/unit -q
uv run ruff check scripts/recover_test_schema.py tests/unit/test_schema_recovery.py
uv run python scripts/recover_test_schema.py --help
uv run python scripts/recover_test_schema.py inspect
```

最初に4つの安全条件テストを作り、未実装による4件の失敗を確認した。その後、模擬テストを12件へ拡充して実装し、既存82件を含め94件の成功を確認。キャンセル・バックアップ失敗・想定外テーブル・誤ったDB・復元先の既存データ・チェックサム不一致などを検証した。差し替え表記は3引数に統一している。

静的チェックとヘルプ表示は成功。実DBの読み取り専用`inspect`では`mondel_test`が空（`{}`）であることを確認した。実DBでのバックアップ・削除・復元は実行していない。したがって、実際のデータを保存して復元する往復検証は未実施であり、重要データの唯一のバックアップ手段としては使わない。
